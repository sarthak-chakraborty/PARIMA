...
player_tiles_x = math.ceil(player_width/width*ncol_tiles)
player_tiles_y = math.ceil(player_height/height*nrow_tiles)
...
def pred_frames(data, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count,frame_nos,total_error1,total_error2):
	series = []
	shift_x = False
	...
	for k in range(len(frames)):
		...
		# print(x_pred,y_pred,x_act,y_act)
		...
		# actual_tile_row = 0 if actual_tile_row < 0 else actual_tile_row
		# actual_tile_col = 0 if actual_tile_col < 0 else actual_tile_col
...
def build_model(data, frame_nos, max_frame):
	...
	while True:
		...
		# Add frames indices that needs to be predicted in next x seconds
		frames = {i}
		...
		metric_X, metric_Y, tile_manhattan_error, count, act_tiles, pred_tiles,total_error1,total_error2 = pred_frames(data, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count,frame_nos,total_error1,total_error2)
		prev_frames = frames
				...
				for k in range(ncol_tiles):
					if not (j==tile_0 and k==tile_1):
						...
						if(dist<=(player_tiles_x+player_tiles_y)/2):
							...
							chunk_weight[j][k] += 1. - (2.0*dist)/((ncol_tiles+nrow_tiles)/2)
...
def main():
	...
	act_tiles,pred_tiles,chunk_frames, manhattan_error, x_mae, y_mae,_,_,_ = build_model(data, frame_nos, max_frame)
	...
	qoe,q1,q2,q3,q4 = calc_qoe(vid_bitrate, act_tiles, frame_nos, chunk_frames)
