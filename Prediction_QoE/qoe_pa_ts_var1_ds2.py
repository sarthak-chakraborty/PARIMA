...
def pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count,total_error1,total_error2):
	...
	for k in range(len(frames)):
		...
		if(k == 0):
			...
			inp_k['VIEWPORT_x'] = x_pred_list[k]
			inp_k['VIEWPORT_y'] = y_pred_list[k]
...
def build_model(data, frame_nos, max_frame, tot_objects):
	model = linear_model.PARegressor(C=0.01, mode=2, eps=0.001, data=data, learning_rate=0.1, rho=0.99)
	...
	while True:
		...
		metric_X, metric_Y, tile_manhattan_error, count, act_tiles, pred_tiles,total_error1,total_error2 = pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count,total_error1,total_error2)
...
def main():
	...
	act_tiles,pred_tiles,chunk_frames, manhattan_error, x_mae, y_mae,_,_,_ = build_model(data, frame_nos, max_frame, tot_objects)
	i = 0
	while True:
		curr_frame=frame_nos[i]
		if curr_frame<5*fps:
