...
class CubemapProjection(AbstractProjection):
  ...
  def reprojectToEquirectangular(self, filepath,sourceProjection):
    ...
    for x in objects:
        ...
        u2 = 2.0*(float(x2)/float(self.imsize[0])-0.5)
        v2 = 2.0*(float(y2)/float(self.imsize[1])-0.5)
        
        if side=='front':
            theta, phi = self.get_theta_phi(halfcubeedge, u1, v1)
            x1eq,y1eq,_ = sourceProjection.pixel_value((theta, phi))

            theta, phi = self.get_theta_phi(halfcubeedge, u2, v1)
            x2eq,y2eq,_ = sourceProjection.pixel_value((theta, phi))

            theta, phi = self.get_theta_phi(halfcubeedge, u1, v2)
            x3eq,y3eq,_ = sourceProjection.pixel_value((theta, phi))

            theta, phi = self.get_theta_phi(halfcubeedge, u2, v2)
            x4eq,y4eq,_ = sourceProjection.pixel_value((theta, phi))
            ...
            theta, phi = self.get_theta_phi(v1, u2, -halfcubeedge)
            x2eq,y2eq,_ = sourceProjection.pixel_value((theta, phi))
            
            theta, phi = self.get_theta_phi(v2, u1, -halfcubeedge)
            x3eq,y3eq,_ = sourceProjection.pixel_value((theta, phi))

            theta, phi = self.get_theta_phi(v2, u2, -halfcubeedge)
            x4eq,y4eq,_ = sourceProjection.pixel_value((theta, phi))
            ...
            theta, phi = self.get_theta_phi(u2, -halfcubeedge, v1)
            x2eq,y2eq,_ = sourceProjection.pixel_value((theta, phi))

            theta, phi = self.get_theta_phi(u1, -halfcubeedge, v2)
            x3eq,y3eq,_ = sourceProjection.pixel_value((theta, phi))

            theta, phi = self.get_theta_phi(u2, -halfcubeedge, v2)
            x4eq,y4eq,_ = sourceProjection.pixel_value((theta, phi))
        ...
        f_out.write(frameno+" "+objtype+" "+str(x1eq)+" "+str(y1eq)+" "+str(x2eq)+" "+str(y2eq)+" "+str(x3eq)+" "+str(y3eq)+" "+str(x4eq)+" "+str(y4eq)+" "+confidence)
