...
def pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tot_objects, tile_manhattan_error, act_tiles, pred_tiles, count):
	series = []
	exog = []
	for k in range(len(prev_frames)):
		[inp_k, x_act, y_act] = data[prev_frames[k]]
		a = [-1 for i in range(2)]
		e = [-1 for i in range(2 * tot_objects)]
	...
	exog = np.array(exog, dtype=np.float64)
	...
	# Storing exogenous variables for forecasting
	exog_forecast = [exog[-1]]
		...
		for key, value in inp_k.items():
			if key != 'VIEWPORT_x' and key != 'VIEWPORT_y':
				...
				if t[2] == 'x':
					...
					e[2*index + 1] = value
		exog_forecast.append(e)
	exog_forecast = np.array(exog_forecast, dtype=np.float64)
...
def build_model(data, frame_nos, max_frame, tot_objects):
	...
	while True:
		...
		metric_X, metric_Y, tile_manhattan_error, count, act_tiles, pred_tiles = pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tot_objects, tile_manhattan_error, act_tiles, pred_tiles, count)
